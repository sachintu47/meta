...
# 4. Generate view of newly released tools from local release cache
if [ -f "tools/create_latest_release_doc.py" ]; then
  python3 tools/create_latest_release_doc.py --input docs/api/zopen_releases.json --output docs/newly_released.md
fi
...
TOKEN="${GITHUB_TOKEN:-${GH_TOKEN:-${ZOPEN_GITHUB_OAUTH_TOKEN:-}}}"
...
if [ -z "${TOKEN}" ]; then
  echo "ERROR: GITHUB_TOKEN / GH_TOKEN / ZOPEN_GITHUB_OAUTH_TOKEN must be defined."
  exit 1
fi
